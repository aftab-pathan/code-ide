...
const NavMain = ({ isGridLayout, setIsGridLayout }) => {
  ...
  useEffect(() => {
    fetch(api_base_url + "/getUserDetails", {
      ...
      body: JSON.stringify({
        userId: localStorage.getItem("codeIdeUserId"),
      }),
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const logout = () => {
    localStorage.removeItem("codeIdeUserId");
    localStorage.removeItem("codeIdetoken");
    localStorage.removeItem("isCodeIdeLoggedIn");
